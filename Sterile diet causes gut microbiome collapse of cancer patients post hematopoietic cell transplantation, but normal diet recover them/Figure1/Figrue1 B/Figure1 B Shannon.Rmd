...
write.csv(ppp, file = "shannon compare.csv", row.names = FALSE)
```
...
p <- ggplot(aa, aes(x= time, y=value ,color=type,fill=type,shape=type))+
  ...
  scale_fill_manual(values=c("#593892"))+
  scale_color_manual(values=c("#593892")) +
  scale_y_continuous(expand = c(0, 0))+
  scale_x_continuous(breaks=seq(0,8,1))+
  geom_line(size=1.5)+
  geom_ribbon(aes(ymin = value-se,
                  ymax = value+se),fill = "#593892",alpha=0.2) +
  geom_point(color="#593892",size=10,shape=19)+
  labs(title="",x="Sampling sites", y = "Shannon diversity index")+
  theme_bw()+
...
p 
ggsave("Figure1 B Shannon.pdf", width =12, height = 6)
```
